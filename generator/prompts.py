...
def draft_prompt(topic: str, keywords: list[str], min_chars: int, max_chars: int) -> str:
    ...
    return dedent(
        ...
        全体を貫く裏テーマ（必須）:
        - 記事のトピックが何であっても、必ず「自分の時間を消耗せず、人間が介在しなくても完全自動化でお金（ポイントや収益）が儲かる仕組み作り」に絡めて論じてください。
        - 読者が「いかに不労所得的な自動化資産を構築するか」という視点を持てるように内容を構成してください。
        - 記事の最後には必ず「本気で自動化・不労所得を構築したい方向けの実践マニュアル」への誘導（CTA: Call To Action）を魅力的なセールスライティングで挿入してください。リンク先は「`/products/`（商品一覧ページ）」を指定してください。
...
def final_check_prompt(improved: str) -> str:
    return dedent(
        ...
        8. 記事内の画像リンク（`![...](https://image.pollinations.ai/...)`）は絶対に削除せず、そのまま残しているか

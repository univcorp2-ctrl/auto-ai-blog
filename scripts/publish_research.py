...
NOTION_DB_ID = "38f10562-e58b-81a5-9b0c-f36281fae520"

RESEARCH_DIR = Path(r"G:\マイドライブ\AI_Agents\Earning_Research")
RESEARCH_DIR.mkdir(parents=True, exist_ok=True)

BLOG_POST_DIR = Path(r"G:\マイドライブ\AI_Agents\github\repos\auto-ai-blog\sites\business\content\posts")
BLOG_POST_DIR.mkdir(parents=True, exist_ok=True)

TITLE = "【完全放置】AIエージェントによる自律型マネタイズモデルの構築と実装の全貌（成功・失敗事例から導く最短ロードマップ）"

DOCUMENT_BODY = """# 【完全放置】AIエージェントによる自律型マネタイズモデルの構築と実装の全貌
...
def save_to_notion(title, content):
    ...
    data = {
        ...
        "properties": {
            ...
            "テーマ": { "select": { "name": "AI副業" } },
            "公開URL": { "url": "G:\\\u30de\u30a4\u30c9\u30e9\u30a4\u30d6\\\\AI_Agents\\\\Earning_Research\\\\AI_Agent_Monetization_Research.md" }
        },
        "children": blocks
    }
    ...
    try:
        with urllib.request.urlopen(req) as response:
            print("Successfully uploaded research to Notion!")
...
def main():
    # 1. Save to G:\マイドライブ\AI_Agents\Earning_Research\AI_Agent_Monetization_Research.md
    local_path = RESEARCH_DIR / "AI_Agent_Monetization_Research.md"
    local_path.write_text(DOCUMENT_BODY, encoding="utf-8")
    print(f"Saved local research file to: {local_path}")
    
    # 2. Upload to Notion
    save_to_notion(TITLE, DOCUMENT_BODY)
    
    # 3. Save as blog post in sites/business/content/posts/
...
{DOCUMENT_BODY}
"""
    blog_post_path = BLOG_POST_DIR / "2026-07-09-ai-agent-fully-automated-monetization-models.md"
    blog_post_path.write_text(blog_body, encoding="utf-8")
    print(f"Saved blog post to: {blog_post_path}")

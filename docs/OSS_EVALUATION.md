...
プライバシー重視のWeb分析基盤で、リポジトリはMITライセンス。将来、複数端末・複数サイトでファネルを統合する段階の移行候補。MVPではサーバー運用を増やさずlocalStorageイベントを採用した。
...
初期は依存を増やさず、Hugo＋Vanilla JS＋Pythonで仮説検証する。月間イベントが増え、複数端末の統合、サーバー側CV、統計検定、リンク短縮が必要になった時点で移行する。
